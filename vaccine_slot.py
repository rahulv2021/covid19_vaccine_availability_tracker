...
class VaccineSlot:
    def __init__(self, date, age_limit, vaccine, state_name, district_name, pincode, center_name, center_id, available_capacity_dose1, available_capacity_dose2):
        self.date = date
    ...
    def get_upsert_query(self):
        query = """
        Insert into vaccine_slot (center_id, pincode, age_limit, vaccine, state_name, district_name, center_name, slot_date, available_capacity_dose1, timestamp_update_dose1, available_capacity_dose2, timestamp_update_dose2) 
        VALUES ({center_id}, {pincode}, {age_limit}, '{vaccine}', '{state_name}', '{district_name}', '{center_name}', '{slot_date}', {available_capacity_dose1}, CURRENT_TIMESTAMP(), {available_capacity_dose2}, CURRENT_TIMESTAMP())
    ...
    def get_updated_records_query(start_time, vaccine, state_name, age_limit, dose_1=False, dose_2=False):
        ...
        if dose_1:
            query += " (timestamp_update_dose1 > '{start_time}' and available_capacity_dose1>5)".format(start_time=start_time)
        if dose_2:
            if dose_1:
                query += " or "
            query += "(timestamp_update_dose2 > '{start_time}' and available_capacity_dose2>5)".format(start_time=start_time)
        query += ")"
