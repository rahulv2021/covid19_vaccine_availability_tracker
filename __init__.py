...
def get_available_slots_by_state_id_and_dates(scheme, hostname, headers, state_id, district_ids, dates, start_time, dose_1=True, dose_2=True, age_limits=[18, 45], vaccines=["covaxin", "covishield", "sputnik"]):
    if not district_ids:
        district_ids = cowin_gateway.get_districts_by_state_id(
            scheme, hostname, headers, state_id)
    available_slots = []
    api_calls = 1
    for date in dates:
        for district_id in district_ids:
            centers = cowin_gateway.get_vaccine_centers_by_district_and_date(
                scheme, hostname, headers, district_id, date)
            api_calls += 1
...
def get_updated_records_from_database(start_time, vaccine, state_name, age_limit, dose_1=False, dose_2=False):
    ...
    query = vaccine_slot.VaccineSlot.get_updated_records_query(
        start_time, vaccine, state_name, age_limit, dose_1, dose_2)
    if query:
        connection = DatabaseConnection().get_connection()
        cursor = connection.cursor()
        cursor.execute(query)
...
if __name__ == "__main__":
    ...
    while True:
            ...
            for vaccine in config.vaccines:
                for age_limit in config.age_limits:
                    for dose in config.vaccines[vaccine][age_limit]:
                        ...
                        updated_slots = get_updated_records_from_database(
                            start_time=start_time, vaccine=vaccine, state_name=state, age_limit=age_limit, **{dose: config.vaccines[vaccine][age_limit][dose]})
        ...
        diff = datetime.datetime.strptime(
            end_time, date_format) - datetime.datetime.strptime(start_time, date_format)
